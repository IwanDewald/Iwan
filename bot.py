...
def redact_order(call):
	if   call.data[0] == '+':
		...
		bot.answer_callback_query(callback_query_id=call.id, show_alert=False, text='+'+str(call.data[-3:])+str('р. к чеку'))
	if call.data[0] == '-':
		storage.remove_from_order(chat_id=call.message.chat.id,data=call.data[1:])
		print(storage.get_user_string(chat_id=call.message.chat.id))
		bot.answer_callback_query(callback_query_id=call.id, show_alert=False, text='-'+str(call.data[-3:])+str('р. из чека'))
